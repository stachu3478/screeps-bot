...
describe('Checking if terminal needs to be filled from storage', () => {
  ...
  describe('Reources to transfer from terminal', () => {
    beforeEach(() => {
      sinon
        .stub(storageManagement, 'shouldBeTakenFromStorage')
        .callsFake((resource: ResourceConstant) =>
          resource === RESOURCE_POWER ? 1 : 0,
        )
      sinon.stub(storageManagement, 'shouldBeTakenToStorage').returns(0)
    })
    ...
    it('Should return true', () => {
      ...
      expect(storageManagement.prepareToTakeResource).to.be.calledWithExactly(
        creep,
        RESOURCE_POWER,
        1,
  ...
  describe('Resources to transfer to terminal', () => {
    beforeEach(() => {
      sinon.stub(storageManagement, 'shouldBeTakenFromStorage').returns(0)
      sinon
        .stub(storageManagement, 'shouldBeTakenToStorage')
        .callsFake((resource: ResourceConstant) =>
          resource === RESOURCE_POWER ? 1 : 0,
        )
    })
