...
describe('Checking if lab needs to be filled with boosting resources', () => {
  ...
  it('should return false for sufficient resources', function () {
    const lab = {
      ...
      store: { [RESOURCE_UTRIUM_ACID]: 300 },
    } as StructureLab
    expect(needsToBeFilledForBoosting(lab, RESOURCE_UTRIUM_ACID, 300)).to.eql(
      false,
    )
  })
  ...
  it('should return false for different resources', function () {
    const lab = {
      mineralType: RESOURCE_UTRIUM_ALKALIDE,
      store: { [RESOURCE_UTRIUM_ALKALIDE]: 300 },
    } as StructureLab
    expect(needsToBeFilledForBoosting(lab, RESOURCE_UTRIUM_ACID, 300)).to.eql(
      false,
    )
  ...
  it('should return false for different insufficient resources', function () {
    const lab = {
      mineralType: RESOURCE_UTRIUM_ALKALIDE,
      store: { [RESOURCE_UTRIUM_ALKALIDE]: 150 },
    } as StructureLab
    expect(needsToBeFilledForBoosting(lab, RESOURCE_UTRIUM_ACID, 300)).to.eql(
      false,
    )
  ...
  it('should return false for full lab', function () {
    const lab = {
      mineralType: RESOURCE_UTRIUM_ACID,
      store: { [RESOURCE_UTRIUM_ACID]: LAB_MINERAL_CAPACITY },
    } as StructureLab
    expect(
      needsToBeFilledForBoosting(lab, RESOURCE_UTRIUM_ACID, Infinity),
    ).to.eql(false)
  })
})
