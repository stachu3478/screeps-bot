...
describe('Checking if lab is used for boosting and does further checks and requests', () => {
  ...
  it('should return false for missing boost data', function () {
    expect(lookForBoosting(creep, [RESOURCE_GHODIUM_ACID, 0], lab)).to.eql(
      false,
    )
  })
  ...
  it('should return true for matching amount and found fill job', function () {
    ...
    expect(lookForBoosting(creep, [RESOURCE_UTRIUM_ACID, 300], lab)).to.eql(
      true,
    )
    expect(labJobs.needsToBeFilledForBoosting).to.be.calledOnceWithExactly(
      lab,
      RESOURCE_UTRIUM_ACID,
      300,
    )
    expect(labJobs.prepareCreepForFilling).to.be.calledOnceWithExactly(
      creep,
      lab,
      RESOURCE_UTRIUM_ACID,
      300,
  ...
  it('should return true for matching index and amount and found dump job', function () {
    ...
    expect(labJobs.needsToBeDumpedForBoosting).to.be.calledOnceWithExactly(
      lab,
      RESOURCE_UTRIUM_ACID,
    )
    expect(labJobs.prepareCreepForDumping).to.be.calledOnceWithExactly(
      creep,
      'lab',
      RESOURCE_UTRIUM_ACID,
    )
