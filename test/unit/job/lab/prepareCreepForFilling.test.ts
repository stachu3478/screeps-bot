...
describe('Preparing creep to fill lab with speciied resources', () => {
  ...
  beforeEach(() => {
    ...
    room.terminal = terminal = { id: 'terminal', store: { [RESOURCE_UTRIUM_ACID]: Infinity } } as StructureTerminal
  });
  ...
  it('should target creep to fill given lab with specified resource', function () {
    ...
    creep.room = room
    const lab = { id: 'theLab', store: { [RESOURCE_UTRIUM_ACID]: 0 } } as StructureLab
    expect(prepareCreepForFilling(creep, lab, RESOURCE_UTRIUM_ACID, 300)).to.eql(true)
  ...
  it('should limit creep resource amount while lab is some filled', function () {
    const creep = { memory: {}, store: { getFreeCapacity: () => Infinity } } as LabManager
    creep.room = room
    const lab = { id: 'theLab', store: { [RESOURCE_UTRIUM_ACID]: 100 } } as StructureLab
    expect(prepareCreepForFilling(creep, lab, RESOURCE_UTRIUM_ACID, 300)).to.eql(true)
  ...
  it('should limit creep resource amount while creep cannot carry everything at once', function () {
    const creep = { memory: {}, store: { getFreeCapacity: () => 200 } } as LabManager
    creep.room = room
    const lab = { id: 'theLab', store: { [RESOURCE_UTRIUM_ACID]: 0 } } as StructureLab
    expect(prepareCreepForFilling(creep, lab, RESOURCE_UTRIUM_ACID, 300)).to.eql(true)
    expect(creep.memory._draw).to.eql('terminal')
  ...
  it('should fail for missing terminal', function () {
    ...
    const lab = { id: 'theLab', store: { [RESOURCE_UTRIUM_ACID]: 0 } } as StructureLab
    expect(prepareCreepForFilling(creep, lab, RESOURCE_UTRIUM_ACID, 300)).to.eql(false)
  ...
  it('should fail for missing resources in terminal with store resources in lab', function () {
    const creep = { memory: {}, store: { getFreeCapacity: () => 50 } } as LabManager
    creep.room = room
    terminal.store[RESOURCE_UTRIUM_ACID] = 99
    const lab = { id: 'theLab', store: { [RESOURCE_UTRIUM_ACID]: 150 } } as StructureLab
    expect(prepareCreepForFilling(creep, lab, RESOURCE_UTRIUM_ACID, 300)).to.eql(false)
    expect(creep.memory._draw).to.be.undefined
    expect(creep.memory._drawAmount).to.be.undefined
    expect(creep.memory._targetLab).to.be.undefined
