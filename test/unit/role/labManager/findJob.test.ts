...
describe('Finding job for lab manager', () => {
  ...
  it('Returns true if boost job found', () => {
    ...
    expect(labJobs.prepareBoostResources).to.be.calledWithExactly(
      creep,
      creep.room.externalLabs,
    )
  })
  ...
  it('Returns false for lab state pending and no such job', () => {
    ...
    expect(findJob(creep)).to.eql(false)
    expect(creep.memory.state).to.eql(State.IDLE)
    expect(labJobs.prepareReaction).to.be.calledWithExactly(
      lab1,
      lab2,
      creep.room.terminal,
      creep.room.memory,
  ...
  it('Returns false for lab state collecting and no such job', () => {
    ...
    expect(labJobs.collectResources).to.be.calledWithExactly(creep, [
      externalLab,
      lab1,
      lab2,
    ])
