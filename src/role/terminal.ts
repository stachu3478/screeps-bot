  ...
  switch (cache.state) {
    ...
    case State.TERM_SEND_EXCESS:
      ...
      switch (sendExcess(term)) {
        case NO_RESOURCE: case NOTHING_TODO: cache.state = State.TERM_BUSINESS; break
        case DONE: case FAILED: cache.state = State.TERM_SELL_EXCESS; break
      }
      break
    case State.TERM_SELL_EXCESS:
      ...
      switch (sellExcess(term)) {
        case SUCCESS: break
        default: cache.state = State.TERM_BUY_MISSING
      }
      break
    case State.TERM_BUY_MISSING:
      ...
      switch (buyMissing(term)) {
        case SUCCESS: break
        default: cache.state = State.TERM_BUSINESS
      }
      break
    case State.TERM_BUSINESS:
      switch (makeBusiness(term)) {
        ...
        case NOTHING_DONE: term.room.visual.text('Terminal: Looking for occasions: ' + RESOURCES_ALL[cache.resourceIteration || -1], 0, 4, infoStyle)
      }
      break
    default: cache.state = State.TERM_BUSINESS
  }
