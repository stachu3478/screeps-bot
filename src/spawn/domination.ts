...
function getOwnedRooms() {
  ...
  return (
    (_.isUndefined(roomCount)
      ? (global.Cache.ownedRooms = Object.keys(Memory.myRooms).length)
      : roomCount) || 0
  )
}
...
function autoClaim(spawn: StructureSpawn, creepCountByRole: number[]) {
  ...
  if (getOwnedRooms() >= Math.min(Game.gcl.level, Memory.roomLimit || 0))
    return false
  if (spawn.room.memory._claim) {
    if (!creepCountByRole[Role.CLAIMER]) {
      spawn.trySpawnCreep(
        progressiveClaimer(spawn.room.energyCapacityAvailable),
        'C',
