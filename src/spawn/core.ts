import { progressiveWorker, progressiveMiner, progressiveLiteWorker } from './body'
import { HARVESTER, UPGRADER, MINER, RETIRED } from '../constants/role'
...
export default function loop(spawn: StructureSpawn, creepCountByRole: number[], workPartCountByRole: number[]) {
  ...
  if (minerCount === 0 && !creepCountByRole[RETIRED]) {
    ...
    spawn.spawnCreep(progressiveMiner(Math.max(SPAWN_ENERGY_START, spawn.room.energyAvailable)), name, { memory: { role: MINER, room: spawn.room.name, _harvest: mem.colonySourceId, deprivity: 0 } })
    mem.creeps[name] = 0
    ...
    spawn.spawnCreep(progressiveLiteWorker(Math.max(SPAWN_ENERGY_START, energyDeclared)), name, { memory: { role: HARVESTER, room: spawn.room.name, deprivity: 0 } })
    mem.creeps[name] = 0
    ...
    if (spawn.room.storage && spawn.room.storage.store[RESOURCE_ENERGY] > (4 + mem.workControllerOver) * CREEP_LIFE_TIME * UPGRADE_CONTROLLER_POWER) {
