...
Room.prototype.setBuildingOrder = function (
  x: number,
  y: number,
  order: number,
) {
  ...
  this.memory.structs = structs
    .replace(char, '')
    .replace(
      charToReplace,
      charPos < order ? charToReplace + char : char + charToReplace,
    )
}

Room.prototype.moveBuilding = function (
  x1: number,
  y1: number,
  x2: number,
  y2: number,
