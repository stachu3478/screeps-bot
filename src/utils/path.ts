...
export function cheapMove(creep: Creep, target: RoomPosition | _HasRoomPosition): ScreepsReturnCode {
  let result = creep.moveTo(target, { noPathFinding: true, reusePath: 100 })
  if (result === ERR_NOT_FOUND) {
    result = creep.moveTo(target, { ignoreCreeps: true, reusePath: 100 })
  }
  ...
  if (dir) {
    ...
    if (creepOnRoad) {
      if (!creepOnRoad.memory) {
        if (!creepOnRoad.my) return creep.moveTo(target)
