...
export default function buyMissing(terminal: StructureTerminal) {
  const missingResource = resourcesToBuy.find(resource => terminal.store[resource] < storageBuyThreshold)
  if (!missingResource) return DONE
  const missingAmount = storageBuyThreshold - terminal.store[missingResource]
  ...
  const best = _.min(orders, (o) => {
    ...
    return TERMINAL_MIN_SEND * o.price + Game.market.calcTransactionCost(TERMINAL_MIN_SEND, terminal.room.name, destRoomName) * energyCost
  })
  const result = Game.market.deal(best.id, Math.min(missingAmount, best.amount), terminal.room.name)
