...
export default function sendExcess(terminal: StructureTerminal) {
  ...
  const resourceType = terminal.room.memory.terminalDealResourceType || RESOURCES_ALL.find(resource => room.store(resource) > storageSendThreshold)
  ...
  for (const name in Memory.myRooms) {
    ...
    const toSend = Math.min(missing, excessLocalAmount, terminal.store[resourceType])
