...
const storageManagement = {
  shouldBeTakenToStorage: (resource: ResourceConstant, terminal: StructureTerminal, storage: StructureStorage) => {
    return Math.max(0, Math.min(terminal.store[resource] - storageBufferingThreshold, storage.store.getFreeCapacity()))
  },

  shouldBeTakenFromStorage: (resource: ResourceConstant, terminal: StructureTerminal, storage: StructureStorage) => {
    return Math.max(0, Math.min(storageBufferingThreshold - terminal.store[resource], storage.store[resource]))
  },
