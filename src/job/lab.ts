...
const labJobs = {
  prepareReaction: (
    ...
    if (
      (lab1.store[resource1] < targetAmount &&
        labJobs.prepareCreepForFilling(creep, lab1, resource1, targetAmount)) ||
      (lab2.store[resource2] < targetAmount &&
  ...
  needsToBeFilledForBoosting: (
    lab: StructureLab,
    resourceToFillWith: ResourceConstant,
    amountToFillWith: number,
  ) => {
    const mineralType = lab.mineralType
    if (!mineralType) return true
    if (lab.mineralType !== resourceToFillWith) return false
    if (
      lab.store[mineralType] < Math.min(amountToFillWith, LAB_MINERAL_CAPACITY)
    )
      return true
    return false
  },
  needsToBeDumpedForBoosting: (
    lab: StructureLab,
    resourceToFillWith: ResourceConstant,
  ) => {
    const mineralType = lab.mineralType
  ...
  lookForBoosting: (
    ...
    if (
      ...
      labJobs.prepareCreepForDumping(
        creep,
        lab.id,
        lab.mineralType as ResourceConstant,
      )
      return true
    }
    return false
